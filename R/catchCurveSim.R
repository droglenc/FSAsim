...
#'
#' A simulated change in instantaneous mortality rate is controlled with the \bold{\sQuote{Z delta}} slider bar, \bold{\sQuote{Steady?}} checkbox, and the \code{ZdeltaAge} arguments. The \bold{\sQuote{Z.delta}} slider controls a proportional adjustment in the mortality rate that begins at \code{ZdeltaAge}. The adjustment is a constant proportion through time/ages if \bold{\sQuote{Steady?}} is checked, but grows geometrically if\bold{\sQuote{Steady?}} is not checked. For example, if \bold{\sQuote{Z mean}} is set to 0.40, \bold{\sQuote{Z cv}} is set to 0 (i.e., no variability), \bold{\sQuote{Z delta}} is set to 1.2, \code{ZdeltaAge=5}, and \bold{\sQuote{Steady?}} is checked then the vector of mortalities used for ages 1 to 10 would be
#'
#' \code{c(0.4,0.4,0.4,0.4,0.4*1.2,0.4*1.2,0.4*1.2,0.4*1.2,0.4*1.2,0.4*1.2)}
#'
#' Alternatively, if all values were the same except that \bold{\sQuote{Steady?}} is not checked, then the vector of mortalities used for ages 1 to 10 would be 
#' 
...
#'
#' Slider bars are used to control similar aspects of the initial population size (No). \bold{\sQuote{No mean}} and \bold{\sQuote{No CV}} are the mean and coefficient of variation to be used in the simulations. Changes in the initial population size are simulated with the \bold{\sQuote{No delta}} slider, \bold{\sQuote{Steady?}} check box, and the \code{NodeltaAge} argument. These adjustments are applied in the same manner as described for the instantaneous mortality rate.
#'
...
#' @param deltaAge A single numeric that indicates the age at which the mortality rate and population size should change. See details.
...
#'@export
#'
catchCurveSim <- function(max.age=15,v=rep(1,max.age),Zycl=FALSE,
  ...
  if (!iCheckRStudio()) FSA:::STOP("'catchCurveSim' only works in RStudio.")
  if (iChk4Namespace("manipulate")) {
    manipulate::manipulate(
      {
        set.seed(sample(1:100000))
        Z.param <- list(mean=Z.mean,cv=Z.cv,delta=Z.delta,deltaAge=deltaAge,dstdy=steady)
        No.param <- list(mean=No.mean,cv=No.cv,delta=No.delta,deltaAge=deltaAge,dstdy=steady)    
        iCC.plot(max.age,v,Zycl,deltaAge,recruit.age,Z.param,No.param)
      },
...
iCC.plot <- function(max.age,v,Zycl,deltaAge,recruit.age,Z.param,No.param) {
