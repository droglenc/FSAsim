...
#
#' For each version, a set of plots is produced that are linked to a set of slider bars that allows the user to change model parameters or create assumption violations.  A slider is created to control the number of removal events, initial population size (No), effort, and catchability coefficent (q).  The remaining sliders allow for simulating specific violations to the assumption of a Leslie model.  These sliders are further described below.
#'
...
#'
#' The \bold{\sQuote{Survival}} value is a constant used as a proportion of fish alive at time t that survive to time t+1 or, if \code{use.rand=TRUE}, is the probability that a fish survives from time t to time t+1.  The survival function is applied to the population after the catch at time t has already been removed from the population.
#'
...
#'
#' @param type A single string that indicates the type or version of simulation that should be used.  See the details.
    ...
    manipulate::manipulate(
      {
        ...
        params.def <- list(k=k,Nt=rep(400,k),Et=rep(5,k),qt=rep(0.05,k),
      ...
      q=manipulate::slider(0.01,0.1,step=0.01,initial=0.05,label="Catchability (q)"),
      ...
      r.prop=manipulate::slider(0,0.2,step=0.05,initial=0.0,label="Prop Recruitment")
    )
  }
}
...
iLeslie.detPlotMain <- function(params.def,params.mod,params.ass,
  ...
  legend("topright",legend=c("assumptions not met","if assumptions met","default"),
...
iLeslie.detPlot <- function(resdef,res1,res2,Ricker.mod=FALSE,glbl) {
  ...
  old.par <- par(mar=c(3.5,3.5,1.5,1.5),mgp=c(2,0.4,0),tcl=-0.2,
                 xaxs="i",yaxs="i")
  plot(cpe~K,data=resdef,type="o",pch=19,lwd=2,col="gray",main=glbl,
  ...
  points(res2$K,res2$cpe,type="o",pch=19,lwd=2,col="red")
  # Plot values for modified parameters but if no assumptions violated
  points(res1$K,res1$cpe,type="o",pch=19,lwd=2,col="blue")
  par(old.par)
} # end iLeslie.detPlot
...
iLeslie.randPlotMain <- function(params,Ricker.mod,q.adj.const) {
  ...
  old.par <- par(mar=c(3.5,3.5,1.5,1.5), mgp=c(2,0.4,0),tcl=-0.2)
  plot(res,xlim=c(0,1000),ylim=c(0,50),
...
iLeslie.mcPlotMain <- function(params,Ricker.mod,rsmpls) {
  ...
  hist(res.N0hat,main="",xlab="Estimated Population Size",yaxt="n",ylab="",xlim=range(c(params$Nt,h.N$breaks)),col="gray90",yaxs="i")
  abline(v=params$Nt[1],col="red",lwd=4,lty=3)
  ...
  hist(res.qhat,main="",xlab="Catchability Estimate",yaxt="n",ylab="",xlim=range(c(params$qt[1],h.q$breaks)),col="gray90",yaxs="i")
  abline(v=params$qt[1],col="red",lwd=4,lty=3)
  abline(v=mean(res.qhat),col="blue",lwd=3)
  # Scatterplot of No vs q
  par(mar=c(3.5,2,1,1), mgp=c(1,0,0)) 
  plot(res.qhat,res.N0hat,xlab="Catchability Estimate",ylab="Estimated Population Size",xaxt="n",yaxt="n",pch=19,xlim=range(c(params$qt[1],res.qhat)),ylim=range(c(params$Nt,res.N0hat)))
  abline(h=params$Nt[1],col="red",lwd=4,lty=3)
  abline(v=params$qt[1],col="red",lwd=3,lty=3)
