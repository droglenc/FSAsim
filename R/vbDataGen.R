...
#' \enumerate{
#'   \item Use \code{Linf}, \code{K}, and \code{t0} to set \sQuote{true} values for the three parameters in a typical VBGF (see \code{growthFunShow} described in \code{\link[FSA]{growthModels}} in \pkg{FSA} for the equation of this parameterization).
...
#    else lenCap <- round(rep(tmp[d$ageCap==d$ageFracY],ages),lendigs)
  ...
  if (dataType=="atCapture")
    d <- subset(d,ageCap==agePrev,c("id","lenCap","ageCap","ageFracG","ageFracY"))
    else if (dataType=="groupedData")
      d <- nlme::groupedData(len~agePrev|id,data=d,labels=list(x="Age",y="Length"))
      else if (dataType=="tagrecap") d <- iCreateTaggedDF(d)
        else if (dataType=="wide") {
          d$agePrev <- paste0("age",d$agePrev)
          d <- tidyr::spread(d[,-which(names(d) %in% c("ageFracG","ageFracY"))],
                             agePrev,len)
        }
  d
}
...
iCreateAges <- function(n,minAge,maxAge,agedist) {
  ...
  # First some checks
  if (minAge<1) 
    FSA:::STOP("'minAge' argument must be greater than or equal to 1.")
  if (minAge>maxAge) 
    FSA:::STOP("'minAge' can NOT be greater than 'maxAge'.")
  if (length(agedist)!=(maxAge-minAge+1)) 
    FSA:::STOP("'agedist' must be same length as 'maxAge'-'minAge'+1.")
...
iCreateFracAges <- function(age,growth.per,sample.per) {
  # check growth.per
  if (length(growth.per)!=2) 
    FSA:::STOP("'growth.per' must have beginning and ending dates.")
  if (any(growth.per<0 | growth.per>365)) 
    FSA:::STOP("'growth.per' values must be between 0 and 365.")
  if (growth.per[1]>growth.per[2]) growth.per <- growth.per[c(2,1)]
  # check sample.per
  if (length(sample.per)==1) sample.per <- rep(sample.per,2)
  if (length(sample.per)!=2)
    FSA:::STOP("'sample.per' must be one date or have beginning and ending dates.")
  if (any(sample.per<0 | sample.per>365))
    FSA:::STOP("'sample.per' values must be between 0 and 365.")
  ...
  # If sampled before growth starts then fraction is 0, if sampled after growth
  # ceases then fraction is 1, if sampled within the growth season then convert
  # to a fraction completed of the the growth seasons
...
iCreateTaggedDF <- function(d) {
  ...
  for (i in 1:length(ids)) {
    ...
    tmp <- tmp[order(tmp)]
    newd[i,] <- c(ids[i],tmpdf$len[tmp[1]],tmpdf$len[tmp[2]],
                  tmpdf$ageFracG[tmp[2]]-tmpdf$ageFracG[tmp[1]])
  }
